...
interface TeamClientProps {
    initialMerchandisers: MerchandiserProfile[];
}

export function TeamClient({ initialMerchandisers }: TeamClientProps) {
    const [searchTerm, setSearchTerm] = useState('');
    const [merchandisers, setMerchandisers] = useState<MerchandiserProfile[]>(initialMerchandisers);
            ...
            {!loading && filteredTeam.length === 0 && (
                <div className="text-center py-12">
                    <p className="text-gray-500 dark:text-gray-400">
                        {searchTerm ? `Aucun membre trouvé pour "${searchTerm}"` : 'Aucun merchandiser. Ajoutez-en un !'}
                    </p>
                </div>
            )}
